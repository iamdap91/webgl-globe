...
let cloudRoatationSpeed = 0.001;
let cameraPos = 1.5;
// 


const guiData = {
    currentURL: '../images/svg/korea.svg',
    drawFillShapes: true,
    drawStrokes: true,
...
async function init2() {
    ...
    const sphere = createSphere(earthParam.radius, earthParam.segments);
    sphere.rotation.y = earthParam.rotation;
    scene.add(sphere)

    const clouds = createClouds(earthParam.radius, earthParam.segments);
    clouds.rotation.y = earthParam.rotation;
    scene.add(clouds)
    ...
    function render() {
        if (cameraAutoRotation) {
            clouds.rotation.y -= cloudRoatationSpeed;
        };
        controls.update();
        updateLabels();
...

    {
        ...
        const texture = loader.load('images/2_no_clouds_4k_test.png', render);
    ...
    // dat.gui
    {
        ...
        const cameraControls = {
            speed: cameraRotationSpeed,
            cloudRoatationSpeed: cloudRoatationSpeed,
            orbitControls: !cameraAutoRotation,
            // cameraPos
        }
        // guiCamera.add(cameraControls, 'speed', 0, 0.1).step(0.001).onChange((value) => cameraRotationSpeed = value);
        guiCamera.add(cameraControls, 'cloudRoatationSpeed', 0, 0.1).step(0.001).onChange((value) => cloudRoatationSpeed = value);
        guiCamera.add(cameraControls, 'orbitControls').onChange((value) => {
        ...
        const surfaceControls = {
            ...
            bumpScale: sphere.material.bumpScale,
            viewClouds: true,
            reverseColor: false,
        }
        // guiSurface.add(surfaceControls, 'rotation', 0, 6).onChange((value) => sphere.rotation.y = value);
        guiSurface.add(surfaceControls, 'bumpScale', 0, 0.2).step(0.01).onChange((value) => sphere.material.bumpScale = value);
        ...
        guiSurface.add(surfaceControls, 'reverseColor').onChange((value) => {
            sphere.material.map = (value) ? THREE.ImageUtils.loadTexture('images/2_no_clouds_4k_test.png') : THREE.ImageUtils.loadTexture('images/2_no_clouds_4k.jpg');
        });
        ...
        guiMode.add(modeControls, 'pickingMode').onChange((value) => {
            if (value) {
                ...
                lightControls.ambientLight = true;
                ...
                lightControls.pointLight = false;

                sphere.material.map = THREE.ImageUtils.loadTexture('images/2_no_clouds_4k_test.png');
            } else {
                sphere.position.z = 0;
                pickingSphere.position.z = 100;
                ...
                lightControls.ambientLight = false;
                ...
                lightControls.pointLight = true;

                sphere.material.map = THREE.ImageUtils.loadTexture('images/2_no_clouds_4k.jpg');
            }
        });
    }
...
function addPicking() {
...

    {
        ...
        // const texture = loader.load('https://threejsfundamentals.org/threejs/resources/data/world/country-outlines-4k.png', render);
        const texture = loader.load('images/2_no_clouds_4k_transparent.png');
        const bumpMap = THREE.ImageUtils.loadTexture('images/elev_bump_4k.jpg');
        const specularMap = THREE.ImageUtils.loadTexture('images/water_4k.png');
        const material = new THREE.MeshBasicMaterial({
            map: texture,
